    ...
    <AnimatePresence>
      {fact && (
        <motion.div
          ...
          onClick={onDismiss}
        >
          <div className="bg-white/95 backdrop-blur-sm shadow-lg rounded-2xl px-5 py-3 border border-gray-100">
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-yellow-100 to-orange-100">
                <Sparkles className="w-5 h-5 text-yellow-500" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-gray-800">新发现！</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${CATEGORY_COLORS[fact.category]}`}>
                    {CATEGORY_LABELS[fact.category]}
                  </span>
                </div>
                <p className="text-sm text-gray-600 mt-0.5">{fact.content}</p>
              </div>
            </div>
          </div>
        </motion.div>
